...
const TempLiveUser = require("../tempLiveUser/tempLiveUser.model");
...
exports.liveUser = async (req, res) => {
  try {
    ...
    console.log("create image and temp");
    ...
    followers.map(async (data) => {
      const payload = {
        notification: {
          body: `${data.to_user_id.name} is live`,
        },
        data: {
          image: image_.image,
      ...
      notification.title = `${data.to_user_id.name} is live now`;
      ...
      admin
        .messaging()
        .sendToDevice(data.from_user_id.fcm_token, payload)
    ...
    if (req.query.name && req.query.user_id) {
      ...
      response.map((res) => {
        if (followers.length > 0) {
          followers.map((data) => {
            if (res._id.toString() === data.to_user_id.toString()) {
              list.push({
                _id: res._id,
                bio: res.bio,
                rate: res.rate,
